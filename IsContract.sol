pragma solidity 0.8.0;
   ...
   /* You shouldn't rely on `isContract` (to check if the call is made only be EOA and not by contract) to protect against flash loan attacks!
    *
...
contract CodeSize {
    ...
    function isContract_new(address account) public view returns (bool) {
        // This method returns 0 for contracts in construction, since the code is only stored at the end of the constructor execution.
        return account.code.length > 0;
    }

    function isContract_old(address _addr) public view returns (bool){
        uint256 size;
        // retrieve the size of the code, this needs assembly, can be bypassed if used during constructor
        assembly {
