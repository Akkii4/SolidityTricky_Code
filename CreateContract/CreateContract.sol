...
contract CreateContract {
    ...
    function getBytecode(uint _testVar) public pure returns (bytes memory){
    ...
    function createBytecode(bytes memory bytecode) external payable{
        ...
        assembly {
            ...
            // n = size of code
            addr := create(callvalue(), // wei value sending as part of the call
    ...
    //   take last 20 bytes
    function preComputeCreate2Address (bytes memory bytecode, uint _salt) public view returns (address) {
    ...
    function create2Bytecode(bytes memory bytecode, uint _salt) external payable{
        address addr;
        assembly {
            // create(v, p, n, s)
            // v= amount of ETH to send
